...
public class Company : MonoBehaviour
{
    ...
    public class Project
    {
        ...
        public class Task
        {
            ...
            public void AssignTo(Character member,Timer.Date startTime)
            {
                owner = member;
                owner.workManager.tasks.Add(this);
                this.startTime = startTime + (int)Mathf.Ceil(member.workManager.workTime);
                float duration = maxProgress / member.lifeController.talentListManager.GetAttributeByEnum(AttributeID.work).value;
                deadLine = startTime + (int)Mathf.Ceil(duration);
                //Debug.Log($"startTime:{startTime.ToMinutes()} deadLine:{deadLine.ToMinutes()}");
                owner.workManager.workTime = deadLine.ToMinutes();
                isAssigned = true;
                //Debug.Log(owner.name + " take " + taskName);
            }
        ...
        public void PM()
        {
            foreach(Module module in modules)
            {
                ...
                foreach(Task task in module.tasks)
                {
                    ...
                    task.AssignTo(member, startTime);
                }
            }
        }
    }
    ...
    void Awake()
    {
        project = new Project("Project A", 1000, Timer.Time.dd + 1, 7, departments[0].groups);
    }
